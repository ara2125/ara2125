...
def gen_st_value_test(
  ...
  return f"""
    ...
    csrr {reg_base1}, mngr2proc < {base}
    {gen_nops(num_nops_base)}
    csrr {reg_base2}, mngr2proc < {result}
    {gen_nops(num_nops_base)}

    # Instruction under test
    {inst} {reg_base2}, {offset}({reg_base1})
    {gen_nops(num_nops_dest)}

    # Check the result
    lw x3, {offset}({reg_base1})
    csrw proc2mngr, x3 > {result}
