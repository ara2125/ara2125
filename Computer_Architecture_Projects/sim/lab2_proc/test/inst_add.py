...
def gen_raw_add_test():
  return """
    # RAW Hazard: x1 is written by first ADD and read by second ADD
    csrr x1, mngr2proc < 5
    csrr x2, mngr2proc < 4
    add x1, x1, x2      # Write to x1
...
def gen_war_add_test():
  return """
    # WAR Hazard: x2 is read by first ADD then written by second ADD
    csrr x1, mngr2proc < 5
    csrr x2, mngr2proc < 4
    add x3, x1, x2      # Read x2
...
def gen_waw_add_test():
  return """
    # WAW Hazard: x1 is written by both ADD instructions
    csrr x2, mngr2proc < 4
    csrr x3, mngr2proc < 3
    add x1, x2, x3      # Write to x1
    add x1, x2, x2      # Write to x1 again
    nop
    nop
    nop
    nop
